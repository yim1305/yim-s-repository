...
void printGPS(int gpsHandle, char* gpsBuffer); // Add function declaration
...
//REAL launch conditions
	...
	if(launched){
	   cout << "launched" << endl;
        ...
        //check if time to process and flush the gps buffer - FIXED ORDER
        ...
        if(gpsTimePassed.count() >= GPS_FLUSHTIME && gpsHandle >= 0){
            // FIRST: Process GPS data before flushing
            printGPS(gpsHandle, gpsBuffer);
            
            // THEN: Flush any remaining data in the buffer
	...
	auto currentTimer = chrono::steady_clock::now();
        ...
        // GPS processing during flight - same fix applied
